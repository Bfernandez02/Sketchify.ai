...
export default function ArtCard({
  ...
  return (
    ...
    >
      ...
      >
        <Link href={`/art/${id}`} className="relative overflow-hidden h-full">
          {loading === "loading" && (
        ...
        <div
          className={`flex h-fit flex-col gap-1 rounded-b-[20px] bg-white p-4 group-data-[simple=true]:hidden ${
            grid ? "w-full" : "absolute bottom-0 w-[250px] md:w-[275px]"
          }`}
        >
          <Link
            href={`/art/${id}`}
          ...
          {categories.length > 0 && (
            <div className="-translate-x-0.5">
              <TagCarousel tags={categories} tagClassName="h-fit text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="">
              ...
              <p className="text-[12px] text-gray-500">Posted {date}</p>
            </div>
          </div>
        </div>
      </div>
